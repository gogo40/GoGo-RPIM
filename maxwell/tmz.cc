...
void* threadH(void* p_args) {
    ...
    updateHTMZ(pts, ini, fim, cte, dt);

    pthread_exit(NULL);
}

bool calcEH(bool isE, point::Points* pts, size_t NTHREADS, const double& dt, const double& cte) {
    size_t N = pts->size();
...
bool tmz(point::Points* pts, const double& Ts, const double& dt,
    ...
    double duracaoPulso, double wfreq) {
    ...
    while (it != end) {
        ...
        if (prop & P_MED) {
            if (type == 'E') {
                std::sprintf(fname,"Ez_%g_%g.dat", x, y);
                ...
                std::sprintf(fname,"Hx_%g_%g.dat", x, y);
                fH[0].push_back(std::fopen(fname,"w+"));

                std::sprintf(fname,"Hy_%g_%g.dat", x, y);
    ...
    for (int t = 0; t < nt; ++t) {
        ...
        //Update Ez
        maxwelltmz::calcEH(true, pts, NTHREADS, dt, cte);
        ...
        //Update Hx and Hy
        maxwelltmz::calcEH(false, pts, NTHREADS, dt, cte);
        ...
        if (videoGen) {
            if (t % nf == 0) {
                ...
                std::sprintf(fname, "ez-%d", ni);
                std::sprintf(com,"t=%e |Ez|=[%e %e]",t * dt, emin, emax);
                levelCurve(nx, ny, image, fname, com, isLog, 1, 0);
                std::sprintf(fname,"convert -level %c%d"
                " ez-%d.png ez-%d.png",'%',lecont,ni,ni);
                std::system(fname);
                std::system("rm *.ppm");
                ++ni;
            }
    ...
    if (videoGen) {
        std::system ("mencoder 'mf://ez-%d.png' -mf fps=10"
        " -o EZ.avi -ovc lavc -lavcopts vcodec=msmpeg4v2:vbitrate=800");
    }
