...
void genPoints(Points* vp, double dt,
    ...
    for (double x = x_upml_1- nupml * 2 * dx; x < x_upml_2 + nupml * 2 *dx; x += dx) {
        for (double y = y_upml_1- nupml * 2 * dx; y < y_upml_2 + nupml * 2 * dx; y += dx) {
            ...
            //if (prop)
            //if (!((x > xf - H - 1e-4) and (x < xf +  H + 1e-4) 
              //      and (y > yf - B - 1e-4) and (y < yf +  B + 1e-4)))

            if (! (inTri(x, y, xt, yt, B, H) and okTri and d > raio - 1e-3 and d < raio + 1e-3)) 
            {
    ...
    for (int i = 0; i < 200; ++i) {
        ...
        fprintf(f1, "%e %e 1\n", x, y);
        fprintf(fmedmap, "Ez_%g_%g.dat med_%d.dat\n", x, y, i);
    }
...
//*/
    ...
    while (!feof(fE)) {
        ...
        if (fabs(x-x1) < 1e-4 and ! ((prop&P_METAL) || (prop&P_PEC))) {
