...
/**Copyright 2012 PÃ©ricles Lopes Machado
@file maxwellTMZ.cc
@brief Neste arquivo sÃ£o declaradas funÃ§Ãµes genÃ©ricas compartilhadas pelos diversos mÃ³dulos do programa


UFPA - Universidade Federal do ParÃ¡
LaboratÃ³rio de AnÃ¡lises NumÃ©ricas em Eletromagnetismo (LANE)

ImplementaÃ§Ã£o do RPIM - Radial Point Interpolation Method

Esta implementaÃ§Ã£o faz parte do TCC de Engenharia da ComputaÃ§Ã£ de PÃ©ricles Lopes Machado (0608000101)
*/
...
double source(const double& t, const double& dt,
    ...
    double eta2 = exp(  - (tau12/tau22)*pow(  nn*tau22/tau12 , 1.0e0/nn )  );
    ...
    switch (type) {
        ...
        case MONOCICLOGAUSS:
            ...
            //sinal = pico * std::exp(-std::pow((t-t0),2)/(tau*tau)); //geracao do monociclogaussiano.
        ...
        case MONOCICLOGAUSSBAVELIS:
            q = t / dt;
            eta = - pow(q - q0, 2) / (2 * W * W);
            tau1 = 2 * M_PI * fc * dt * (q - q0);
...
int main(int argc, char** argv) {
    ...
    if (type == MONOCICLOGAUSS || type == GAUSS) {
        printf("tau?"); scanf("%lf", &tau);
        printf("t0?"); scanf("%lf", &t0);

    } else if (type == MONOCICLOGAUSSBAVELIS) {


        printf("q0?"); scanf("%lf", &q0);
        printf("W?"); scanf("%lf", &W);
        printf("fc?"); scanf("%lf", &fc);
    }
    ...
    lane_maxwell::modules::maxwell::tmz(p, Ts, dt, cte,
    resX, resY, (videoGen)? true: false,
    nf, isLog? true: false, lecont, NTHREADS, source,
    sourceType, duracao, 2*acos(-1)*fc);
